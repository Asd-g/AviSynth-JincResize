...
//        iMul = static_cast<int64_t>(iMultH);
    ...
    else if (avx2)
    {
        if (bAddProc)
        {
            if (ap == 1)
            {
                if (iMul == 4 && iTaps == 4)  KernelProcAll = &JincResize::KernelRowAll_avx2_mul4_taps4; 
...
#pragma omp parallel num_threads(threads_)
	{
		...
		int64_t iStartRow =/* iTaps + */tidx * iNumRowsPerThread/* - iTaps*/;
		int64_t iThreadSkipRows = iTaps * 2;
		// some check
		if (iStartRow < iTaps) iStartRow = iTaps;
		int64_t iEndRow = iStartRow + iNumRowsPerThread + iTaps;
